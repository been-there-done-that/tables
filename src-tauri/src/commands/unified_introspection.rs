...
pub async fn refresh_schema_unified(
    ...
    let orchestrator_config = IntrospectorConfig::new(&connection_id)
        .with_cache(true)
        .with_events(ENABLE_INTROSPECTION_EVENTS)
    ...
    let orchestrator = ProgressiveIntrospector::new(adapter, orchestrator_config)
        ...
        .with_event_callback(Box::new(move |event| {
            ...
            let event_name = match &event {
                ...
                IntrospectionEvent::SchemaProgress { .. } => "introspection:progress",
                IntrospectionEvent::TableDiscovered { .. } => "introspection:table_discovered",
                IntrospectionEvent::TableComplete { .. } => "introspection:table_complete",
            };
    ...
    match options.scope {
        ...
        IntrospectionScope::Schema { database, schema } => {
            orchestrator.introspect_schema(&database, &schema).await
                .map_err(|e| format!("Introspection failed: {:?}", e))?;
            // Cache saving is handled by introspect_schema
        },
