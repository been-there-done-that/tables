...
pub trait DatabaseAdapter: Send + Sync {
    ...
    /// List all columns for an entire schema (Bulk).
    /// Default implementation iterates list_tables -> list_columns.
    async fn list_columns_schema(&self, database: &str, schema: &str) -> Result<Vec<MetaColumn>, AdapterError> {
        ...
        for table in tables {
            let table_ref = TableRef::new(database, schema, &table.table_name);
            let columns = self.list_columns(&table_ref).await?;
            all_columns.extend(columns);
        }
        Ok(all_columns)
    }

    /// List all indexes for an entire schema (Bulk).
    async fn list_indexes_schema(&self, database: &str, schema: &str) -> Result<Vec<MetaIndex>, AdapterError> {
        ...
        for table in tables {
            let table_ref = TableRef::new(database, schema, &table.table_name);
            let indexes = self.list_indexes(&table_ref).await?;
            all_indexes.extend(indexes);
        }
        Ok(all_indexes)
    }

    /// List all foreign keys for an entire schema (Bulk).
    async fn list_foreign_keys_schema(&self, database: &str, schema: &str) -> Result<Vec<MetaForeignKey>, AdapterError> {
        ...
        for table in tables {
            let table_ref = TableRef::new(database, schema, &table.table_name);
            let fks = self.list_foreign_keys(&table_ref).await?;
            all_fks.extend(fks);
        }
        Ok(all_fks)
    }

    /// List all triggers for an entire schema (Bulk).
    async fn list_triggers_schema(&self, database: &str, schema: &str) -> Result<Vec<MetaTrigger>, AdapterError> {
        ...
        for table in tables {
            let table_ref = TableRef::new(database, schema, &table.table_name);
            let triggers = self.list_triggers(&table_ref).await?;
