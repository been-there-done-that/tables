# PostgreSQL Sprint Integration Tests — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** One integration test that creates a known schema in a real Postgres DB and verifies all 11 new PostgreSQL browse + DDL commands return correctly shaped data.

**Architecture:** Single `#[tokio::test]` function in a new file `src-tauri/src/commands/pg_sprint_tests.rs`, following the `postgres_diagnostic_test.rs` pattern. Connects via `DATABASE_URL` env var (defaults to `postgres://postgres:postgres@localhost:5432/postgres`). Creates a fixed test schema, runs each command's SQL inline, asserts results, drops schema.

**Tech Stack:** Rust, tokio-postgres (already a dependency), `#[tokio::test]`
...
## Task 1: Write and run the integration test
...
Open `src-tauri/src/commands/mod.rs` and add this line after the last `pub mod` at the bottom of the file:
...
- [ ] **Step 2: Verify it compiles before writing the test**

```bash
cd src-tauri && cargo check 2>&1 | grep "^error"
```

Expected: no output (no errors). If you see "file not found", create an empty `pg_sprint_tests.rs` first:
```bash
touch src-tauri/src/commands/pg_sprint_tests.rs
```
Then re-run `cargo check`.

- [ ] **Step 3: Write the full test file**

Create `src-tauri/src/commands/pg_sprint_tests.rs` with this exact content:
...
mod tests {
    ...
    async fn test_pg_sprint_introspection() {
        ...
        // ── 1. get_functions ──────────────────────────────────────────────────
        // Same query as introspection_commands.rs::get_functions
        ...
        // get_user_email (function) and log_user_change (trigger fn) and mark_user_active (procedure)
        ...
        // ── 2. get_sequences ─────────────────────────────────────────────────
        // Proves data_type::text cast works (pg_sequences.data_type is regtype)
        let rows = client.query(
        ...
        assert_eq!(rows.len(), 1, "expected 1 sequence, got {}", rows.len());
        ...
        // Expect at least: primary key index + partial index
        ...
        // Build DDL string the same way as the command
...
```bash
cd src-tauri && cargo test test_pg_sprint_introspection -- --nocapture 2>&1
```

Expected output (when Postgres is running):
```
✓ get_functions: found 3 functions/procedures
✓ get_sequences: custom_id_seq data_type=bigint
✓ get_constraints: ["users_age_positive", "users_email_unique"]
✓ get_index_details: found indexes [...]
✓ get_table_ddl:
CREATE TABLE "tables_sprint_test"."users" (
    ...
)
✓ get_view_definition: OK
✓ get_matview_definition: populated=true
✓ get_function_ddl: OK
✓ get_sequence_ddl:
CREATE SEQUENCE "tables_sprint_test"."custom_id_seq" ...
✓ get_index_ddl: CREATE INDEX idx_users_age_partial ON ...
✓ get_trigger_definition: CREATE TRIGGER users_audit AFTER INSERT OR UPDATE ON ...
...
```
[pg_sprint_tests] Postgres not available (...), skipping.
test commands::pg_sprint_tests::tests::test_pg_sprint_introspection ... ok
```

- [ ] **Step 5: Fix any failing assertions**

If an assertion fails, the message includes the actual value. Common issues:

- `data_type not 'bigint'` → the `::text` cast is missing from the query. Check the `get_sequences` SQL.
- `partial index not found` → check that the `tables_sprint_test` schema was created correctly. Re-run setup manually.
- `TRIGGER ... AFTER INSERT OR UPDATE` assertion fails → Postgres stores the trigger definition with events in alphabetical order or separated differently. Adjust the assertion to just check for `TRIGGER users_audit`.

- [ ] **Step 6: Commit**

```bash
git add src-tauri/src/commands/pg_sprint_tests.rs src-tauri/src/commands/mod.rs
git commit -m "test(postgres): integration tests for all 11 sprint browse + DDL queries"
```
...
**Spec coverage check:**
- ✅ `get_functions` — queries + kind assertion (prokind 'f' vs 'p')
- ✅ `get_sequences` — data_type::text cast verified (was a runtime panic risk)
- ✅ `get_constraints` — CHECK and UNIQUE both present and typed correctly
- ✅ `get_index_details` — btree type + partial predicate verified
- ✅ `get_table_ddl` — identity, generated, CHECK, UNIQUE all in output
- ✅ `get_view_definition` — CREATE OR REPLACE VIEW header present
- ✅ `get_matview_definition` — CREATE MATERIALIZED VIEW + WITH DATA
- ✅ `get_function_ddl` — pg_get_functiondef returns CREATE OR REPLACE FUNCTION
- ✅ `get_sequence_ddl` — double-quoted identifiers + correct values
- ✅ `get_index_ddl` — pg_indexes.indexdef contains CREATE INDEX + partial WHERE
- ✅ `get_trigger_definition` — pg_get_triggerdef returns TRIGGER name + event

**Placeholder scan:** None found. All SQL is verbatim from the production commands.

**Type consistency:** All queries use the same column indices and Rust types (`i8` for prokind/contype, `&str` for text, `bool` for flags, `i64` for int8) as the production command code.
