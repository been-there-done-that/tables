<script lang="ts">
    ...
    import { ENGINE_SCHEMAS, createEmptyConfig, isFieldVisible } from "$lib/schema/connectionSchema";
...
<div class="space-y-4 text-sm">
    ...
    {#if tab === "general"}
        <div class="grid grid-cols-[120px_1fr] gap-y-3 items-center">
            <label for="db.host" class="text-[--theme-fg-secondary]">Host:</label>
            <div class="flex space-x-2">
                <div class="grow">
                    <FormInput
                        ...
                        oninput={(e: any) => updateField("db.host", e.target.value)}
                    />
                    {#if getFieldError("db.host")}
                        <div class="text-xs text-red-600 mt-1">{getFieldError("db.host")}</div>
                    {/if}
                </div>

                <div class="flex items-center space-x-2">
                    <label for="db.port" class="text-[--theme-fg-secondary]">Port:</label>
                    <div class="w-32">
                        <FormInput
                            inputId="db.port"
                            type="number"
                            ...
                            placeholder="5432"
                            oninput={(e: any) => updateField("db.port", parseInt(e.target.value))}
                        />
                        {#if getFieldError("db.port")}
                            <div class="text-xs text-red-600 mt-1">{getFieldError("db.port")}</div>
                        {/if}
                    </div>
                </div>
            </div>

            <label for="db.database" class="text-[--theme-fg-secondary]">Database:</label>
            <FormInput
                inputId="db.database"
            ...
            {#if getFieldError("db.database")}
                <div class="text-xs text-red-600 mt-1">{getFieldError("db.database")}</div>
            {/if}

            <label for="db.username" class="text-[--theme-fg-secondary]">User:</label>
            <FormInput
                inputId="db.username"
            ...
            {#if getFieldError("db.username")}
                <div class="text-xs text-red-600 mt-1">{getFieldError("db.username")}</div>
            {/if}

            <!-- Transport Type -->
            <div>
                <label for="transport-type-select" class="text-[--theme-fg-secondary]">Connection:</label>
                <Select
                    id="transport-type-select"
                    value={data.transport?.type || 'direct'}
                    onCommit={(value: string) => updateField("transport.type", value)}
                    options={[
                ...
                {#if getFieldError("transport.type")}
                    <div class="text-xs text-red-600 mt-1">{getFieldError("transport.type")}</div>
                {/if}
            </div>
            ...
            <div>
                ...
                <label class="flex items-center space-x-2">
                    <input
                        ...
                        onchange={(e: any) => updateField("tls.enabled", e.target.checked)}
                        class="rounded"
                    />
                ...
                {#if data.tls?.enabled}
                    <div class="pl-6 space-y-2">
                        <div>
                            <label for="tls-sslmode-select" class="text-[--theme-fg-secondary]">SSL Mode:</label>
                            <Select
                                id="tls-sslmode-select"
                                value={data.tls?.sslmode || 'prefer'}
                                onCommit={(value: string) => updateField("tls.sslmode", value)}
                                options={[
                            ...
                            {#if getFieldError("tls.sslmode")}
                                <div class="text-xs text-red-600 mt-1">{getFieldError("tls.sslmode")}</div>
                            {/if}
                        </div>
                    </div>
                {/if}
    ...
    {:else if tab === "ssh"}
        {#if data.transport?.type === 'ssh'}
            <div class="grid grid-cols-[120px_1fr] gap-y-3 items-center">
                <label class="text-[--theme-fg-secondary]" for="transport.ssh.host">SSH Host:</label>
                <FormInput
                    inputId="transport.ssh.host"
                    value={data.transport.ssh?.host || ''}
                    placeholder="bastion.example.com"
                    oninput={(e: any) => updateField("transport.ssh.host", e.target.value)}
                />
                {#if getFieldError("transport.ssh.host")}
                    <div class="text-xs text-red-600 mt-1">{getFieldError("transport.ssh.host")}</div>
                {/if}

                <label class="text-[--theme-fg-secondary]" for="transport.ssh.port">SSH Port:</label>
                <FormInput
                    inputId="transport.ssh.port"
                    type="number"
                    value={String(data.transport.ssh?.port || 22)}
                    placeholder="22"
                    oninput={(e: any) => updateField("transport.ssh.port", parseInt(e.target.value))}
                />
                {#if getFieldError("transport.ssh.port")}
                    <div class="text-xs text-red-600 mt-1">{getFieldError("transport.ssh.port")}</div>
                {/if}

                <label class="text-[--theme-fg-secondary]" for="transport.ssh.user">SSH User:</label>
                <FormInput
                    inputId="transport.ssh.user"
                    value={data.transport.ssh?.user || ''}
                    oninput={(e: any) => updateField("transport.ssh.user", e.target.value)}
                />
                {#if getFieldError("transport.ssh.user")}
                    <div class="text-xs text-red-600 mt-1">{getFieldError("transport.ssh.user")}</div>
                {/if}

                <div>
                    <label for="ssh-auth-select" class="text-[--theme-fg-secondary]">SSH Auth:</label>
                    <Select
                    id="ssh-auth-select"
                    value={data.transport.ssh?.auth?.type || 'key'}
                    onCommit={(value: string) => updateField("transport.ssh.auth.type", value)}
                    options={[
                ...
                {#if getFieldError("transport.ssh.auth.type")}
                    <div class="text-xs text-red-600 mt-1">{getFieldError("transport.ssh.auth.type")}</div>
                {/if}
                </div>

                {#if data.transport.ssh?.auth?.type === 'key'}
                    <div>
                        <label class="text-[--theme-fg-secondary]" for="transport.ssh.auth.key_ref">SSH Key Ref:</label>
                        <FormInput
                        inputId="transport.ssh.auth.key_ref"
                        type="password"
                        value={data.transport.ssh.auth?.key_ref || ''}
                        placeholder="Key reference from secure store"
                        oninput={(e: any) => updateField("transport.ssh.auth.key_ref", e.target.value)}
                    />
                    {#if getFieldError("transport.ssh.auth.key_ref")}
                        <div class="text-xs text-red-600 mt-1">{getFieldError("transport.ssh.auth.key_ref")}</div>
                    {/if}
                    </div>
                {:else if data.transport.ssh?.auth?.type === 'password'}
                    <div>
                        <label class="text-[--theme-fg-secondary]" for="transport.ssh.auth.password_ref">Password Ref:</label>
                        <FormInput
                        ...
                        oninput={(e: any) => updateField("transport.ssh.auth.password_ref", e.target.value)}
                    />
                    {#if getFieldError("transport.ssh.auth.password_ref")}
                        <div class="text-xs text-red-600 mt-1">{getFieldError("transport.ssh.auth.password_ref")}</div>
                    {/if}
                    </div>
                {/if}
            </div>
        {:else}
            <div class="text-center py-8 text-[--theme-fg-tertiary]">
                ...
                <p>SSH tunnel is not enabled. Switch to "Direct Connection" in the General tab to use SSH.</p>
            </div>
        {/if}
    {:else if tab === "advanced"}
        <div class="space-y-4">
            <div class="grid grid-cols-[120px_1fr] gap-y-3 items-center">
                <label for="options.search_path" class="text-[--theme-fg-secondary]">Search Path:</label>
                <FormInput
                    inputId="options.search_path"
                    value={data.options?.search_path || 'public'}
                    oninput={(e: any) => updateField("options.search_path", e.target.value)}
                />
                {#if getFieldError("options.search_path")}
                    <div class="text-xs text-red-600 mt-1">{getFieldError("options.search_path")}</div>
                {/if}

                <label for="options.application_name" class="text-[--theme-fg-secondary]">Application Name:</label>
                <FormInput
                    inputId="options.application_name"
                    value={data.options?.application_name || ''}
                    oninput={(e: any) => updateField("options.application_name", e.target.value)}
                />
                {#if getFieldError("options.application_name")}
                    <div class="text-xs text-red-600 mt-1">{getFieldError("options.application_name")}</div>
                {/if}
            </div>
        </div>
    {/if}
